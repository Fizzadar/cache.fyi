...
`);let h=c.textContent.replace(/\u00A0/g," ");u.textContent+=h,c.remove();continue}let p=null;if(c.tagName==="DIV"&&(p=c.querySelector("li")),p){let u=p.classList.contains("bullet-list"),h=p.classList.contains("ordered-list");if(!u&&!h){n=null,i=null;continue}let m=u?"ul":"ol";(!n||i!==m)&&(n=document.createElement(m),t.insertBefore(n,c),i=m);let g=[];for(let f of c.childNodes)if(f.nodeType===3&&f.textContent.match(/^\u00A0+$/))g.push(f.cloneNode(!0));else if(f===p)break;g.forEach(f=>{p.insertBefore(f,p.firstChild)}),n.appendChild(p),c.remove()}else n=null,i=null}return t.innerHTML}static postProcessHTMLManual(e){let t=e;t=t.replace(/((?:<div>(?:&nbsp;)*<li class="bullet-list">.*?<\/li><\/div>\s*)+)/gs,i=>{let r=i.match(/<div>(?:&nbsp;)*<li class="bullet-list">.*?<\/li><\/div>/gs)||[];return r.length>0?"<ul>"+r.map(a=>{let l=a.match(/<div>((?:&nbsp;)*)<li/),c=a.match(/<li class="bullet-list">.*?<\/li>/);if(l&&c){let d=l[1];return c[0].replace(/<li class="bullet-list">/,`<li class="bullet-list">${d}`)}return c?c[0]:""}).filter(Boolean).join("")+"</ul>":i}),t=t.replace(/((?:<div>(?:&nbsp;)*<li class="ordered-list">.*?<\/li><\/div>\s*)+)/gs,i=>{let r=i.match(/<div>(?:&nbsp;)*<li class="ordered-list">.*?<\/li><\/div>/gs)||[];return r.length>0?"<ol>"+r.map(a=>{let l=a.match(/<div>((?:&nbsp;)*)<li/),c=a.match(/<li class="ordered-list">.*?<\/li>/);if(l&&c){let d=l[1];return c[0].replace(/<li class="ordered-list">/,`<li class="ordered-list">${d}`)}return c?c[0]:""}).filter(Boolean).join("")+"</ol>":i});let n=/<div><span class="code-fence">(```[^<]*)<\/span><\/div>(.*?)<div><span class="code-fence">(```)<\/span><\/div>/gs;return t=t.replace(n,(i,r,s,a)=>{let c=(s.match(/<div>(.*?)<\/div>/gs)||[]).map(h=>h.replace(/<div>(.*?)<\/div>/s,"$1").replace(/&nbsp;/g," ")).join(`
...
`:i,b=ne(g)&&a&&a.length>0?`
${a}`:r;if(c){let x=o.value[o.selectionStart-1];o.selectionStart!==0&&x!=null&&!x.match(/\s/)&&(f=` ${f}`)}g=Ke(o,f,b,e.multiline);let y=o.selectionStart,k=o.selectionEnd,M=l&&l.length>0&&b.indexOf(l)>-1&&g.length>0;if(p){let x=J(o);t=x.newlinesToAppend,n=x.newlinesToPrepend,f=t+i,b+=n}if(g.startsWith(f)&&g.endsWith(b)){let x=g.slice(f.length,g.length-b.length);if(h===m){let C=h-f.length;C=Math.max(C,y),C=Math.min(C,y+x.length),y=k=C}else k=y+x.length;return{text:x,selectionStart:y,selectionEnd:k}}else if(M)if(d&&d.length>0&&g.match(d)){b=b.replace(l,g);let x=f+b;return y=k=y+f.length,{text:x,selectionStart:y,selectionEnd:k}}else{let x=f+g+b;return y=y+f.length+g.length+b.indexOf(l),k=y+l.length,{text:x,selectionStart:y,selectionEnd:k}}else{let x=f+g+b;y=h+f.length,k=m+f.length;let C=g.match(/^\s*|\s*$/g);if(u&&C){let G=C[0]||"",X=C[1]||"";x=G+f+g.trim()+b+X,y+=G.length,k-=X.length}return{text:x,selectionStart:y,selectionEnd:k}}}function ae(o,e){let{prefix:t,suffix:n,surroundWithNewlines:i}=e,r=o.value.slice(o.selectionStart,o.selectionEnd),s=o.selectionStart,a=o.selectionEnd,l=r.split(`
